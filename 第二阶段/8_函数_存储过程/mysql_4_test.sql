...
create function st1() returns int
begin
insert into class_2 (name,age,sex,score) values ('Jame',16,'m',88);
update class_2 set score=99 where id=1;
return (select score from class_2 limit 1);
end $$

# 使用变量获取值
create function score_d(id1 int,id2 int)
returns int
begin
declare a float;
declare b float;
set a=(select score from class_2 where id=id1);
select score from class_2 where id =id2 into b;
return a+b;
end $$
...
delimiter $$
create procedure get_age(in id1 int,out age1 int)
begin
    set age1=(select age from class_2 where id=id1) ;
end $$
